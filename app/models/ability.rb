class Ability
  ...
  def initialize(user)
    ...
    user.roles.try(:each) do |role|
      role.permissions.try(:each) do |permission|
        if permission.object_class == 'all'
          can permission.action.squish.to_sym, permission.object_class.squish.to_sym
        else
          can permission.action.to_sym, "Devx::#{permission.object_class.squish.camelize}".constantize
        end
      end
    end
