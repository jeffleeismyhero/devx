module Devx
  class Schedule < ActiveRecord::Base
    ...
    scope :upcoming, -> { where('(start_time < ? OR start_time >= ?) AND end_time >= ?', CURRENT_TIME,CURRENT_TIME,CURRENT_TIME).ordered }
    scope :coming_up, -> { where('start_time >= ?', CURRENT_TIME).ordered }
